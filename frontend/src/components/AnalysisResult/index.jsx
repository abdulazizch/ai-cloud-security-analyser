...
import { Search, Download, FileText, ChevronDown, ChevronRight, BarChart3, CheckCircle } from 'lucide-react';

const AnalysisResult = ({ analysisResults, isDarkMode, fileName}) => {
  ...
  const handleExport = async (format) => {
    try {
      const response = await fetch(`http://localhost:8000/export-report/analysis_results/${format}`);
      ...
      setModalMessage(`Report exported successfully as ${format.toUpperCase()}`);
      setShowModal(true);
    } catch (err) {
      console.error("Export error:", err);
      setModalMessage(`Failed to export report as ${format.toUpperCase()}`);
      setShowModal(true);
    }
  };
  ...
  return (
    <div className={`backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6 ${isDarkMode
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold flex items-center">
          ...
          {fileName ? fileName : 'Analysis Results'}
        </h2>

        <div className="flex space-x-3">
          <button
            onClick={() => handleExport("pdf")}
            className="px-4 py-2 bg-red-500/20 text-red-400 border border-red-500/30 rounded-lg hover:bg-red-500/30 transition-all duration-200 flex items-center space-x-2"
          >
          ...
          <button
            ...
            className="px-4 py-2 bg-blue-500/20 text-blue-400 border border-blue-500/30 rounded-lg hover:bg-blue-500/30 transition-all duration-200 flex items-center space-x-2"
          >
      ...
      <div className="flex space-x-4 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className={`w-full pl-10 pr-4 py-2 rounded-lg  border focus:outline-none focus:ring-2 focus:ring-blue-500/50 ${isDarkMode
        ...
        <select
          ...
          className={`px-4 py-2 rounded-lg focus:outline-none border focus:ring-2 focus:ring-blue-500/50 ${isDarkMode
            ? 'bg-gray-800/50 border-gray-700'
            : 'bg-white/70 border-gray-200'
            }`}
        >
      ...
      <div className="space-y-4">
        {filteredResults.map((issue) => (
          <div key={issue.id} className={`rounded-xl border overflow-hidden ${isDarkMode
            ? 'bg-gray-800/50 border-gray-700'
            : 'bg-white/70 border-gray-200'
            }`}>
            <div
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50">
          <div className={`p-6 rounded-2xl max-w-md w-full text-center ${isDarkMode ? "bg-gray-800 text-white" : "bg-white text-gray-800"}`}>
